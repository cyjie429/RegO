...
from avalanche.training.plugins import StrategyPlugin, CWRStarPlugin, \
    ...
    GSS_greedyPlugin, LFLPlugin, OWMPlugin, RAWMPlugin, RWMPlugin, RegOPlugin
...
class RegO(BaseStrategy):
    ...
    def __init__(self, model: Module, optimizer: Optimizer, criterion, ef_thresh, importnat_thresh,
                 alpha: Union[float, Sequence[float]], temperature: float,
                 train_mb_size: int = 1, train_epochs: int = 1,
                 eval_mb_size: int = None, device=None,
                 plugins: Optional[List[StrategyPlugin]] = None,
        ...
        rego = RegOPlugin(model=model, ef_thresh=ef_thresh, importnat_thresh=importnat_thresh)
        if plugins is None:
            plugins = [awm]
        else:
            plugins.append(awm)
